...
**Decision:** Fixed-width versioned `RearrangementBatchV1` encoded as a QUA-native homogeneous `int` vector, not a mixed packed binary struct. Header fields are `protocol_version`, `sequence_no`, `n_moves`, `deadline_ppu_ticks_lo/hi`, `snapshot_hash64_lo/hi`, `descriptor_hash64_lo/hi`, `loop_index`, `max_loops`, `ideal_moves`, and reserved `header_flags`; each move contributes fixed integer words for source/target site IDs, `group_id`, ramp ticks, and flags. PPU validates every field before playing, including that `loop_index < max_loops`, `max_loops` matches the compiled shot policy, and `n_moves <= N_MAX_MOVES`. `ideal_moves > N_MAX_MOVES` is allowed as a truncation signal and persisted as `truncated = ideal_moves > n_moves`.
**Alternatives:**
...
**Evidence:** QM input-stream docs require pushed data size/type to match the declared input stream; client stream lists are homogeneous typed values. Phase 0A W0A-1 will measure the chosen fixed-width int-vector layout.
**Consequences:** N_MAX_MOVES is part of the accepted contract but remains provisional until Phase 0A derives it from descriptor geometry + assignment/collision policy and proves the target `BATCH_WORDS` declaration compiles, runs, and stays inside the latency budget. Accepting ADR-0002 freezes the measured value; later changes require a new ADR.
...
**Decision:** Local fsync spool on broker → `raw_state = 'raw_spooled'` → idempotent gRPC `ShotResult` → DB tx inserts row + manifest in one tx → `raw_state = 'metadata_mirrored'` → async data-lake/off-host replication → off-host ack → `raw_state = 'replicated'` and shot `state = 'committed'`. Per §05.
**Alternatives:**
...
**Consequences:** Spool can grow if Postgres or off-host replica is delayed; bounded by Tower NVMe; alarming surfaces lag. Execution may complete before durable commit, so UI must distinguish execution completion from `committed`.
